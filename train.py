...
def model_train(
    ...
    for i in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            for batch in tqdm(valid_loader, "validation"):
                model.eval()
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, "test"):
        ...
        print(
            "Epoch {}\t\tTRAIN LOSS: {:.5f}\tVALID AUC: {:.5f}\tTEST AUC: {:.5f}\tTEST ACC: {:.5f}\tTEST RMSE: {:.5f}".format(
                i, train_loss, valid_auc, test_auc, test_acc, test_rmse
            )
        )
