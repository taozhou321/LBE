...
from .nhgp_module import NHGProcessModule

class AKT_LBE2(Module):
    def __init__(self, num_questions, num_skills, num_problems, device, **kwargs):
        super(AKT_LBE2 , self).__init__()
        ...
        self.behavior_proj = nn.Linear(2 * self.hidden_size + 2 * self.hidden_size, 2 * self.hidden_size)

        self.time_attempt_module = NHGProcessModule(self.num_problems, dim=self.hidden_size)
        self.time_hint_moudle = NHGProcessModule(self.num_problems, dim=self.hidden_size)
    ...
    def forward(self, batch):
        ...
        time_hint_factor = self.time_hint_moudle(problems, time_taken, hint_count)
        time_attempt_factor = self.time_attempt_module(problems, time_taken, attempt_count)
        behaviors = [input_x, # (bs, seq_len, 2 * self.hidden_size)
                    time_hint_factor, # (bs, seq_len) --> (bs, seq_len, 1)
                    time_attempt_factor
                    ]
